...
if(!isset($admin_id)){
   ...
   exit(); // Kanunay butangi og exit human sa header location
};

// I-initialize ang $message isip usa ka empty array sa sinugdanan
$message = []; 
...
if(isset($_POST['update_order'])){
   ...
   // ✅ KUNG CANCELLED, I-RESTORE ANG STOCK
...
<head>
   ...
   <style>
      /* Imong existing styles nagpabilin... */
      ...
      .box{ background:rgba(255,255,255,0.08); backdrop-filter:blur(15px); border-radius:20px; padding:20px; box-shadow:0 10px 30px rgba(0,0,0,0.4); transition:0.3s; }
      .box p{ font-size:14px; margin:6px 0; color:#ddd; }
      .box span{ color:#fff; font-weight:500; }
      .price{ font-size:20px; color:#00f260; font-weight:700; margin:10px 0; }
      .drop-down{ width:100%; padding:10px; border:none; border-radius:10px; margin:10px 0; }
      .flex-btn{ display:flex; gap:10px; }
      .option-btn, .delete-btn{ flex:1; text-align:center; padding:10px; border-radius:10px; text-decoration:none; font-weight:600; cursor:pointer; }
      .option-btn{ background:#3498db; color:white; border:none; }
...
<?php
// ✅ FIX: Siguroha nga array ang $message ug dili kini empty sa dili pa mag-foreach
